...
$('body').on('click', '#save_map', async function () {
    ...
    for (let row = ur-1, i=uc-1; i<=lc+1; i++) {
        ...
        await tileDownloader.downloadTile(zoom_level, row, i, turl, 'osm', mapName);
    }
    tile_coords[zoom_level]
    // bottom row
    for (let row=lr+1, j=uc-1; j<=lc+1;j++) {
        let turl = `${tile_str}/${zoom_level}/${row}/${j}.png`;
        await tileDownloader.downloadTile(zoom_level, row, j, turl, 'osm', mapName);
    }
    // left side
    for (let col=uc-1, k=ur; k<=lr; k++) {
        let turl = `${tile_str}/${zoom_level}/${k}/${col}.png`;
        await tileDownloader.downloadTile(zoom_level, k, col, turl, 'osm', mapName);
    }
    // right side
    for (let col=lc+1, n=ur; n<=lr; n++) {
        let turl = `${tile_str}/${zoom_level}/${n}/${col}.png`;
        await tileDownloader.downloadTile(zoom_level, n, col, turl, 'osm', mapName);
    }
